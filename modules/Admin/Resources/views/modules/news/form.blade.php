...
{!! ul_tab_begin() !!}
{!! li_tab(['id'=> 'generalTab', 'title'=> 'Thông tin cơ bản (*)'], true) !!}
{!! li_tab(['id'=> 'filterTab', 'title'=> 'Phân loại danh mục tin (*)']) !!}
{!! ul_tab_close() !!}
<div class="tab-content">
    ...
    <div role="tabpanel" class="tab-pane fade in active" id="generalTab">
        <div class="row">
            ...
            <div class="col-md-4">
                <section id="avatar">
                    <h4 class="text-title">Hình ảnh minh họa
                        <small class="text-muted">(Chọn hình ảnh)</small>
                    </h4>
                    <hr>
                    <div class="jumbotron" style="padding:10px">
                        ...
                        <div class="container-image text-center">
                            <div style="width: 100%;height: 250px; overflow:hidden;position: relative"
                                 class="recipe_show_image img {{ !$newPicture ? 'hidden' : '' }}">
                                <img src="{{ $newPicture }}" alt="" id="avatar_show_image">
                                <div class="image__button editor__tool">
                                    <label class="image_upload" title="Sửa ảnh" for="new_picture_face"><i
                            ...
                            <div class="recipe_image_default {{ $newPicture ? 'hidden' : '' }}">
                                <h1 title="Vui lòng up hình minh họa">
                                    <label style="cursor: pointer" for="new_picture_face">
                                        <img src="{{ asset('backend/imgs/avatar_cook.png') }}" id="img_new_picture"
                                             width="230">
                                    </label>
                                </h1>
                                <input type="hidden" name="new_picture" id="new_picture" value="{{ $newPicture }}">
                                <input type="file" class="hide" accept="image/*" id="new_picture_face"
                                       name="new_picture_face">
                            </div>
                        </div>
                    </div>
                </section>
            ...
            <div class="col-md-8">
                ...
                <section id="ingredient">
                    <h4 class="text-title">Nguyên liệu
                        <small class="text-muted">(Thông tin bắt buộc)</small>
                    </h4>
                    <hr>
                    <div class="form-group {{ has_error($errors, 'new_ingredient') }}">
                        <label class="control-label">Chuẩn bị nguyên liệu{!! label_danger() !!}
                            <small class="text-muted">(Đặt mỗi nguyên liệu trên một dòng)</small>
                        </label>
                        <textarea name="new_ingredient" id="new_ingredient" class="form-control" rows="6"
                                  placeholder="Đặt mỗi nguyên liệu trên một dòng">{!! get_value_field('new_ingredient', $new) !!}</textarea>
                ...
                <section id="methods">
                    ...
                    <div class="form-group">
                        <a href="#" style="color: #54667a" class="text-center btn btn-link btn-block button_add_step"
                           id="button_add_step" data-behavior="add_step">
    ...
    {{-- content tab 2 --}}
    <div role="tabpanel" class="tab-pane fade" id="filterTab">
        <span class="help-block text-danger">{{ $errors->first('new_cate_id')  }}  </span>
        <h4 class="text-title">Thông tin cơ bản </h4>
        <p class="text-muted">Sẽ giúp cho người dùng tìm thấy công thức dễ dàng hơn</p>

        @foreach($categories as $category)
            @if($category['cate_parent_id'] == 0)
                <div class="form-group col-md-6">
                    <h3 class="text-title">{{ $category['cate_name'] }}</h3>
                    <ul class="nav navbar-stacked">
                        @foreach($categories as $cate)
                            @if($cate['cate_parent_id'] == $category['cate_id'])
                                <li class="col-md-6">
                                    <span>
                                        <input type="checkbox"
                                                 {{ in_array($cate['cate_id'], $tintucActiveArr) ? 'checked' : '' }}
                                                 name="new_cate_id[]" value="{{ $cate['cate_id'] }}">
                                        {{ $cate['cate_name'] }}
                                    </span>
                                </li>
                            @endif
